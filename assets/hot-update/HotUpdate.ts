import { Asset, error, log, sys } from 'cc';
...
class Hot {
    ...
    _hotUpdateCallBack(event: jsb.EventAssetsManager) {
        ...
        switch (code) {
            ...
            case jsb.EventAssetsManager.ASSET_UPDATED:
                ...
                break;
            default:
                log(`error code msg: ${codeMsg[code.toString()]}`);
    ...
    _onUpdateFinished(event: jsb.EventAssetsManager) {
        ...
        log("[HotUpdate] 搜索路径: " + JSON.stringify(newPaths));
    ...
    init(manifest: Asset, opt: HotOptions) {
        ...
        this._assetsMgr = new jsb.AssetsManager(url, storagePath, (versionA, versionB) => {
            // 比较版本
            log("客户端版本: " + versionA + ', 当前最新版本: ' + versionB);
        ...
        log('[HotUpdate] 热更新资源存放路径: ' + storagePath);
        log('[HotUpdate] 本地manifest路径: ' + url);
        log('[HotUpdate] local packageUrl: ' + localManifest.getPackageUrl());
        log('[HotUpdate] project.manifest remote url: ' + localManifest.getManifestFileUrl());
        log('[HotUpdate] version.manifest remote url: ' + localManifest.getVersionFileUrl());
    }
}
